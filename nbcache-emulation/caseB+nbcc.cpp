...
#define RTT 50000        // congestion control RTT
...
struct para_congestion{  // parameter for congestion control
    int curnum;
    int cwnd;
    pthread_mutex_t mut;
};
struct para_data_detec{  // parameter for detect data packet
    QueueOper *shm;
    key_t queKey;
    struct para_congestion *pcon;
};
struct edata{            // parameter for aio command
    int pid;
    key_t queKey;
    QueueOper *shm;
    int congestion;
...
struct edata eio_data[REQ_NUM-1];

void *cs_action(void *args);       // cs thread function
void *cs_action_end(void *args);   // cs thread function for the last router r4
void *pit_fib(void *args);         // pit/fib thread function
void *data_pro(void *args);        // data processing thread function
void *check_io(void *args);        // aio polling funtion
void *detec_data_fun(void *args);  // data packet detecting function
void *print_cwnd_fun(void *args);  // cwnd printing function
...
//            usleep(1);
    ...
    if(r2 == 0){
        ...
        bloom_parameters parameters;
        // How many elements roughly do we expect to insert?
        parameters.projected_element_count = 150;
        // Maximum tolerable false positive probability? (0,1)
        parameters.false_positive_probability = 0.000001; // 1 in 1000000
        if (!parameters){
            cout<<"Error - Invalid set of bloom filter parameters!"<<endl;
    ...
    if(r4 == 0){
        ...
        bloom_parameters parameters;
        // How many elements roughly do we expect to insert?
        parameters.projected_element_count = 500;
        // Maximum tolerable false positive probability? (0,1)
        parameters.false_positive_probability = 0.000001; // 1 in 1000000
        if (!parameters){
            cout<<"Error - Invalid set of bloom filter parameters!"<<endl;
        ...
        // create threads
        pthread_t cs_fun, fib_fun, data_fun, check_fun;
        pthread_create(&cs_fun, NULL, cs_action_end, &p1);
        pthread_create(&fib_fun, NULL, pit_fib, &p2);
        pthread_create(&check_fun, NULL, check_io, NULL);
        pthread_join(cs_fun, NULL);
        pthread_join(fib_fun, NULL);
    ...
    if(pkt_gen == 0){
        sleep(1);
        printf("into pkt_gen\n");
        struct para_congestion p_congestion;
        p_congestion.curnum = 0;
        p_congestion.cwnd = 1;
        pthread_mutex_init(&(p_congestion.mut), NULL);
        ...
        struct para_data_detec p1;
        p1.shm = &que3;
        p1.queKey = tQueueKey3;
        p1.pcon = &p_congestion;
        pthread_t detec_data, printcwnd;
        pthread_create(&detec_data, NULL, detec_data_fun, &p1);    // create data packet detecting thread
        pthread_create(&printcwnd, NULL, print_cwnd_fun, &p_congestion);  // create cwnd printing thread
        ...
        for(int i = 0; i<mem_len; i++){
            if(memory[i] != 10){
                ...
                while(1){
                    pthread_mutex_lock(&(p_congestion.mut));
                    if(p_congestion.curnum <= p_congestion.cwnd){    
                        ...
                        strSend.clear();
                        count++;
                        p_congestion.curnum = p_congestion.curnum + 1;
                        pthread_mutex_unlock(&(p_congestion.mut));
                        break;
                    }
                    pthread_mutex_unlock(&(p_congestion.mut));
                }
...
// cwnd printing function
void *print_cwnd_fun(void *q){
    struct para_congestion *para;
    para = (struct para_congestion *)q;
    // for(int i=0; i<30000; i++){
    //     printf("cwnd curnum %d %d\n", para->cwnd, para->curnum);
    //     usleep(250);
    // }
    return 0;
}

// data packet detecting function
void *detec_data_fun(void *q){
    struct para_data_detec *para;
    para = (struct para_data_detec *)q;
    int res;
    ...
    while(1){
        ...
        while(pres < 0){          // read data packet from neighbor_que
            pres = para->shm->ReadDataFromQueueHead(b, 4200);
        }
        req = b;

        pthread_mutex_lock(&(para->pcon->mut));
        para->pcon->curnum = para->pcon->curnum - 1;
        if(para->pcon->curnum < 0){
            printf("curnum error!\n");
            return 0;
        }
        int conges_flag = req[req.length()-1] - '0';
//        printf("congestion flag %d\n", conges_flag);
        if(conges_flag == 0){
            para->pcon->cwnd++;
            pthread_mutex_unlock(&(para->pcon->mut));
        }else if(conges_flag == 1){
            para->pcon->cwnd = para->pcon->cwnd/2;
            pthread_mutex_unlock(&(para->pcon->mut));

            struct timeval start;
            gettimeofday(&start,NULL);
            unsigned long s = 1000000 * start.tv_sec + start.tv_usec;
            during = 0;

            while(during < RTT){
                string req2;
                char *b2;
                b2 = (char*)malloc(4200*sizeof(char));
                int pres2 = -1;
                while(pres2 < 0){          // read data packet from neighbor_que
                    pres2 = para->shm->ReadDataFromQueueHead(b2, 4200);
                }
                req2 = b2;

                struct timeval end;
                gettimeofday(&end,NULL);
                unsigned long e = 1000000 * end.tv_sec + end.tv_usec;
                during = e - s;
                ...
                int conges_flag2 = req2[req2.length()-1] - '0';
                //printf("congestion flag %d\n", conges_flag);
                if(conges_flag2 == 0){
                    para->pcon->cwnd++;
                    pthread_mutex_unlock(&(para->pcon->mut));
                }else if(conges_flag2 == 1){
                    pthread_mutex_unlock(&(para->pcon->mut));
                }else{
                    printf("congestion flag2 error!\n");
                }
            }
        }else{
            printf("congestion flag error!\n");
        }
    }
    return 0;
}

void *check_io(void *q){
    while(1){
        while(eio_npending()){
            eio_poll();
        }
    ...
    while(1){
        ...
        if(res == 1){
            ...
            if(cur_que < IO_QUE){
                ...
                if(offset <= flength){
                    eio_data[pid].queKey = para->queKey2;
                    eio_data[pid].shm = para->shm2;
                    eio_data[pid].congestion = 0;
                    eio_data[pid].quelock = para->quelock;
                    eio_data[pid].pkt_start = pkt_start;
                    // initiate aio command
                	eio_read(io_file, &pbuf[pid][0], SEG_SIZE, offset, 0, call_back, &(eio_data[pid]));
                    continue;
                }else{
...
// cs thread function for the last router r4
void *cs_action_end(void *q){
    struct para_intrstin *para;
    para = (struct para_intrstin *)q;
    int res;
    int cur_que;
...
int call_back(eio_req *req){
    struct edata * k;
    k = (struct edata *)req->data;

    char data_return[SEG_SIZE+1];
    for(int i=0;i<SEG_SIZE;i++){
        data_return[i] = pbuf[k->pid][i];
    }
    if(k->congestion == 1){
        data_return[SEG_SIZE] = '1';
    }else if(k->congestion == 0){
        data_return[SEG_SIZE] = '0';
    }else{
        printf("call back error!\n");
        return 0;
    }
    int r = -1;
    pthread_mutex_lock(k->quelock);
    while(r < 0){
        r = k->shm->AttachQueue(k->queKey);
        if(r < 0){
            printf("call back error\n");
        }
        r = k->shm->PushDataIntoQueueBack(data_return, strlen(data_return));   // return data packet
        if(r < 0){
            printf("call back pushque error\n");
        }
    }
    pthread_mutex_unlock(k->quelock);
    struct timeval back;
    gettimeofday(&back,NULL);
    unsigned long back_t = 1000000 * (back.tv_sec)+ back.tv_usec;
    unsigned long p_latency = back_t - k->pkt_start;
    printf("pid %d %lu\n", k->pid, p_latency);
    return 0;
}
