...
//            usleep(1);
    ...
    if(r1 == 0){
        ...
        // create threads
        pthread_t cs_fun, fib_fun, data_fun;
...
//        printf("congestion flag %d\n", conges_flag);
        if(conges_flag == 0){
            ...
            printf("congestion flag error! %c %d\n", req[req.length()-1], conges_flag);
        }
    }
    return 0;
}
...
void *cs_action(void *q){
    ...
    while(1){
        ...
        int pres = -1;
        while(pres < 0){       // read interest packet from neighbor_que and get neighbor_que length
            pres = para->shm1->AttachQueue(para->queKey1);
            quelength = para->shm1->GetInQueuePkgNum();
        ...
        if(iter != url_file.end()){
            // if content in CS, check neighbor_que length vs threshold
            if(quelength < IO_QUE){
                ...
                if(flag == 0){
                    // if interest flag=0
                    data_return[SEG_SIZE-1] = '0';
                }else if(flag == 1){
                    // if interest flag=1
                    data_return[SEG_SIZE-1] = '1';
                }else{
                    printf("flag error\n");
                    continue;
                }
